    ...
    <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <p class="mt-1">{{ translate('Are you sure to duplicate this product?') }}</p>
                <button type="button" class="btn btn-link mt-2" data-dismiss="modal">{{ translate('Cancel') }}</button>
                <a href="" id="comfirm-link" class="btn btn-primary mt-2 ">{{ translate('comfirm') }}</a>
            </div>
        </div>
    </div>
</div>
    ...
    <form class="" id="sort_products" action="" method="GET">
        <div class="card-header row gutters-5">
            ...
            @if(get_setting('product_approve_by_admin') == 1 && $type == 'Seller')
                <div class="col-md-2 ml-auto">
                    <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="approved" id="approved" onchange="sort_products()">
                        <option value=""selected>{{ translate('All Product') }}</option>
                        <option value="1"  @isset($approved) @if($approved == '1' ) selected @endif @endisset>{{ translate('Approved') }}</option>
                        <option value="0" @isset($approved) @if($approved == '0' ) selected @endif @endisset>{{translate('Not Approved')}}</option>
                    </select>
                </div>
            @endif
            ...
            <div class="col-md-1 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="published" id="published" onchange="sort_products()">
                    <option value=""selected>{{ translate('All Product') }}</option>
                    <option value="1"  @isset($published) @if($published == '1' ) selected @endif @endisset>{{ translate('Published') }}</option>
                    <option value="0" @isset($published) @if($published == '0' ) selected @endif @endisset>{{translate('Not Published')}}</option>
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="lg">{{translate('New Product')}}</th>
                        <th data-breakpoints="lg">{{translate('Best Product')}}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_new_product(this)" value="{{ $product->id }}" type="checkbox" <?php if ($product->new_product == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_best_product(this)" value="{{ $product->id }}" type="checkbox" <?php if ($product->best_product == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td class="text-right">
                            ...
                            @endif
{{--                            <a class="btn btn-soft-warning btn-icon btn-circle btn-sm confirm-complete" data-href="{{route('products.duplicate', ['id'=>$product->id, 'type'=>$type]  )}}" title="{{ translate('Duplicate') }}">--}}
...
{{--                            </a>--}}
                            ...
                            @php
                                ...
                                //   $arr = ['id'=>$product->id];
                                //   isset($type) ? $arr['type'] = $type : false;
                               //    isset($col_name) ? $arr['col_name'] = $col_name : false;
                               //    isset($query) ? $arr['query'] = $query : false;
                                //   isset($sort_search) ? $arr['sort_search'] = $sort_search : false;
                                //   isset($seller_id) ? $arr['seller_id'] = $seller_id : false;
                                 //  isset($approved) ? $arr['approved'] = $approved : false;
                                 // dd($arr);
                            @endphp
                            <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('products.destroy', ['id'=>$product->id])}}" title="{{ translate('Delete') }}">
...
@section('script')
    <script type="text/javascript">
        ...
        function update_new_product(el){
            ...
            $.post('{{ route('products.new_product') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
        ...
        function update_best_product(el){
            if(el.checked){
                var status = 1;
            }
            else{
            ...
            $.post('{{ route('products.best_product') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Best products updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
